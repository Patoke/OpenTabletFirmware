import sys

should_encrypt = sys.argv[1] == "-e"
file_name = sys.argv[2] if should_encrypt else sys.argv[1]

encryption_map = [
    
]

substitution_map = {val: idx for idx, val in enumerate(encryption_map)}

in_file = open(file_name, "rb")
data = bytearray(in_file.read())
in_file.close()

for i in range(len(data)):
    byte_val = data[i]

    new_val = substitution_map.get(byte_val, 0) if not should_encrypt else encryption_map[byte_val]
    data[i] = new_val
    
out_file = open(("dec_" if not should_encrypt else "enc_") + file_name.replace("dec_", ""), "wb")
out_file.write(data)
out_file.close()